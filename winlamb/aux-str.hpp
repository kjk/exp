...
namespace wl::str {
	...
	/// Returns the index of the first occurrence of `what` in `s`, or -1 if
	/// not found.
	///
	/// Starts searching from the offset `off`.
	[[nodiscard]] int index(WStrView s, WStrView what, size_t off = 0);

	/// Returns the index of the last occurrence of `what` in `s`, or -1 if
	/// not found.
	///
	/// Starts searching from the offset `off`.
	[[nodiscard]] int index_rev(WStrView s, WStrView what, size_t off = std::wstring::npos);
