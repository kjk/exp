...
namespace _wl_internal {
	...
	struct RawOrDlg final {
		[[nodiscard]] constexpr const WndBase& base() const {
			return raw
				? raw->_rawBase._wndBase
				: dlg->_dlgBase._wndBase;
		}
		...
		~RawOrDlg() { delete raw; delete dlg; }
...
namespace wl {
	...
	public:
		...
		virtual void ui_thread(Func0 cb) const = 0;
	...
	public:
		...
		explicit WindowMain(MainOpts creationOpts)
			: _rawOrDlg{.raw = new _wl_internal::RawMain(creationOpts)} { }
		...
		explicit WindowMain(WORD dlgId, WORD iconId = 0, WORD accelTblId = 0)
			: _rawOrDlg{.dlg = new _wl_internal::DlgMain(dlgId, iconId, accelTblId)} { }
		...
		/// [`SendMessage`]: https://learn.microsoft.com/en-us/windows/win32/api/winuser/nf-winuser-sendmessagew
		/// [`std::thread`]: https://en.cppreference.com/w/cpp/thread/thread.html
		void ui_thread(Func0 cb) const override { _rawOrDlg.base().ui_thread(cb); }
	...
	public:
		...
		WindowModal(const IWindowParent &parent, ModalOpts creationOpts)
			: _rawOrDlg{.raw = new _wl_internal::RawModal(parent.base(), creationOpts)} { }
		...
		WindowModal(const IWindowParent &parent, WORD dlgId)
			: _rawOrDlg{.dlg = new _wl_internal::DlgModal(parent.base(), dlgId)} { }
	...
	public:
		...
		void on_drop(Func1<const std::vector<std::wstring>&> cb) { _cb = cb; }

	private:
		std::vector<std::wstring> get_dropped(HANDLE hDrop) const;

		LONG _refCount = 1;
		Func1<const std::vector<std::wstring>&> _cb{};
	};
...
namespace wl::msg_box {
	...
	///
	/// If the user cancels, returns an empty `std::wstring`.
	///
	/// Example:
	///
	/// ```cpp
	/// std::wstring f = wl::msg_box::open_file(wnd, {
	///     {L"Text file", L"*.txt"},
	///     {L"All files", L"*.*"},
	/// });
	/// ```
	[[nodiscard]] std::wstring open_file(const IWindowParent &parent,
	...
	///
	/// ```cpp
	/// std::wstring f = wl::msg_box::open_folder(wnd);
	/// ```
	[[nodiscard]] std::wstring open_folder(const IWindowParent &parent);
	...
	///
	/// ```cpp
	/// std::wstring f = wl::msg_box::save_file(wnd);
	/// ```
	[[nodiscard]] std::wstring save_file(const IWindowParent &parent,
		std::initializer_list<std::pair<WStrView, WStrView>> fileTypes);
