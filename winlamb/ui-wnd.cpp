...
int WindowMain::run(HINSTANCE hInst, int cmdShow) {
	return _rawOrDlg.raw
		? _rawOrDlg.raw->run(hInst, cmdShow)
		: _rawOrDlg.dlg->run(hInst, cmdShow);
}
...
WindowControl::WindowControl(IWindowParent &parent, ControlOpts creationOpts)
	: _rawOrDlg{.raw = new RawControl(parent.base(), creationOpts)}
{
	on().wm(WM_NCPAINT, MkFunc1(+[](WindowControl* self, wm::Msg& m) { wm::NcPaint p{m}; self->paint_custom_border(p); }, this));
}

WindowControl::WindowControl(IWindowParent &parent, WORD dlgId, WORD ctrlId, POINT pos, Lay layout)
	: _rawOrDlg{.dlg = new DlgControl(parent.base(), dlgId, ctrlId, pos, layout)}
{
	on().wm(WM_NCPAINT, MkFunc1(+[](WindowControl* self, wm::Msg& m) { wm::NcPaint p{m}; self->paint_custom_border(p); }, this));
}
...
DropFiles::DropFiles(IWindowParent &owner) {
	struct Ctx { DropFiles* self; IWindowParent* owner; };
	auto* c = new Ctx{this, &owner};
	owner.base()._preEvents.wm_create_or_init_dialog(MkFunc0(+[](Ctx* c) {
		RegisterDragDrop(c->owner->hwnd(), c->self);
		delete c;
	}, c));

	owner.base()._postEvents.wm(WM_DESTROY, MkFunc1(+[](IWindowParent* p, wm::Msg&) {
		RevokeDragDrop(p->hwnd());
	}, &owner));
}
...
HRESULT STDMETHODCALLTYPE DropFiles::Drop(
	IDataObject *pDataObj, DWORD grfKeyState, POINTL pt, DWORD *pdwEffect)
{
	if (_cb.IsValid()) { // do we have an user callback?
		FORMATETC fetc{
			.cfFormat = CF_HDROP,
			.ptd = nullptr,
		...
		_cb.Call(files); // invoke user callback
	}
...
std::wstring wl::msg_box::open_file(const IWindowParent &parent,
	std::initializer_list<std::pair<WStrView, WStrView>> fileTypes)
{
	...
	return fd_open(ifod.ptr(), parent) ? fd_get_file(ifod.ptr()) : L"";
}
...
std::wstring wl::msg_box::open_folder(const IWindowParent &parent) {
	ComPtr<IFileOpenDialog> ifod{};
	ifod.co_create_instance(CLSID_FileOpenDialog);
...
std::wstring wl::msg_box::save_file(const IWindowParent &parent,
	std::initializer_list<std::pair<WStrView, WStrView>> fileTypes)
{
	ComPtr<IFileSaveDialog> ifsd{};
	ifsd.co_create_instance(CLSID_FileSaveDialog);
	fd_set_types(ifsd.ptr(), fileTypes);
	return fd_open(ifsd.ptr(), parent) ? fd_get_file(ifsd.ptr()) : L"";
}
