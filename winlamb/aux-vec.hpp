...
namespace wl::vec {
	...
	/// ```cpp
	/// std::vector<int> nums{1, 2, 3, 4};
	/// int idx2 = wl::vec::index(nums, 2);
	/// ```
	template<
		std::ranges::contiguous_range R,
		typename T = std::remove_reference_t<std::ranges::range_reference_t<R>>
	> requires std::ranges::sized_range<R>
	[[nodiscard]] constexpr int index(R &&v, const std::type_identity_t<T> &elem) {
		auto foundIt = std::find(v.begin(), v.end(), elem);
		return (foundIt == v.end()) ? -1 : static_cast<int>(std::distance(v.begin(), foundIt));
	}

	/// Returns the first index according to `pred`, or -1 if not found.
	///
	/// Example:
	///
	/// ```cpp
	/// std::vector<int> nums{1, 2, 3, 4};
	/// int idx2 = wl::vec::index_if(nums, [](const int &n) -> bool {
	///     return n == 2;
	/// });
	/// ```
	template<
		...
		typename T = std::remove_reference_t<std::ranges::range_reference_t<R>>
	> requires std::ranges::sized_range<R>
	[[nodiscard]] int index_if(R &&v, std::predicate<T> auto &&pred) {
		auto foundIt = std::find_if(v.begin(), v.end(), pred);
		return (foundIt == v.end()) ? -1 : static_cast<int>(std::distance(v.begin(), foundIt));
	}

	/// Returns the index of the last found element, or -1 if not found.
	///
	/// Example:
	///
	/// ```cpp
	/// std::vector<int> nums{1, 2, 3, 4};
	/// int idx2 = wl::vec::index_rev(nums, 2);
	/// ```
	template<
		std::ranges::contiguous_range R,
		typename T = std::remove_reference_t<std::ranges::range_reference_t<R>>
	> requires std::ranges::sized_range<R>
	[[nodiscard]] constexpr int index_rev(R &&v, const std::type_identity_t<T> &elem) {
		auto foundIt = std::find(v.rbegin(), v.rend(), elem);
		return (foundIt == v.rend()) ? -1 : static_cast<int>(std::distance(foundIt, std::prev(v.rend())));
	}

	/// Returns the last index according to `pred`, or -1 if not found.
	///
	/// Example:
	///
	/// ```cpp
	/// std::vector<int> nums{1, 2, 3, 4};
	/// int idx2 = wl::vec::index_rev_if(nums, [](const int &n) -> bool {
	///     return n == 2;
	/// });
	/// ```
	template<
		...
		typename T = std::remove_reference_t<std::ranges::range_reference_t<R>>
	> requires std::ranges::sized_range<R>
	[[nodiscard]] int index_rev_if(R &&v, std::predicate<T> auto &&pred) {
		auto foundIt = std::find_if(v.rbegin(), v.rend(), pred);
		return (foundIt == v.rend()) ? -1 : static_cast<int>(std::distance(foundIt, std::prev(v.rend())));
	}
	...
	/// std::vector<int> nums{1, 2, 3, 4};
	/// const int SEQ[] = {2, 3};
	/// int idx = wl::vec::index_seq(nums, SEQ);
	/// ```
	template<
		std::ranges::contiguous_range R,
		std::ranges::contiguous_range Q
	> requires std::ranges::sized_range<R>
		&& std::ranges::contiguous_range<Q>
		&& std::same_as<std::ranges::range_value_t<R>, std::ranges::range_value_t<Q>>
	[[nodiscard]] constexpr int index_seq(R &&v, Q &&sequence) {
	...
	/// ```cpp
	/// std::vector<int> nums{1, 2, 3, 4};
	/// int idx = wl::vec::index_seq(nums, {2, 3});
	/// ```
	template<
		std::ranges::contiguous_range R,
		typename T = std::remove_reference_t<std::ranges::range_reference_t<R>>
	> requires std::ranges::sized_range<R>
	[[nodiscard]] constexpr int index_seq(
			R &&v, std::initializer_list<const std::type_identity_t<T>> sequence) { // initializer_list overload
	...
	[[nodiscard]] std::vector<std::span<T>> split(
			R &&v, const std::type_identity_t<T> &delimiter, int maxParts = -1) {
		if (v.empty()) return {};

		std::span<T> src{v};
		size_t count = 0, head = 0;
		for (;;) {
			if (maxParts > 0 && count >= static_cast<size_t>(maxParts) - 1)
				break;

			int foundHead = index(src, delimiter);
			if (foundHead == -1) {
				break;
			} else {
				head = static_cast<size_t>(foundHead);
				++count;
				++head;
		...
		head = 0;
		for (;;) {
			if (maxParts > 0 && ret.size() >= static_cast<size_t>(maxParts) - 1)
				break;
