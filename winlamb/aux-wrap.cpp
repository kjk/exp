...
void wl::path::dir_list(WStrView dirPath, Func1<const std::wstring&> cb) {
	...
	for (;;) {
		if (!str::eq(wfd.cFileName, L".") && !str::eq(wfd.cFileName, L"..")) { // skip these
			...
			cb.Call(retFileBuf); // invoke user callback
		}
...
std::vector<std::wstring> wl::path::dir_list(WStrView dirPath) {
	...
	dir_list(dirPath, MkFunc1(+[](std::vector<std::wstring>* v, const std::wstring &p) {
		v->emplace_back(p);
	}, &ret));
	return ret;
}

void wl::path::dir_walk(WStrView dirPath, Func1<const std::wstring&> cb) {
	auto entries = dir_list(dirPath);
	for (auto& p : entries) {
		if (is_dir(p)) {
			dir_walk(p, cb); // recursively
		} else {
			cb.Call(p); // invoke user callback
		}
	}
}
...
std::vector<std::wstring> wl::path::dir_walk(WStrView dirPath) {
	std::vector<std::wstring> ret{};
	dir_walk(dirPath, MkFunc1(+[](std::vector<std::wstring>* v, const std::wstring &p) {
		v->emplace_back(p);
	}, &ret));
	return ret;
}
